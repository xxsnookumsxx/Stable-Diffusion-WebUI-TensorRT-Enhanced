...
class Engine:
    ...
    def refit_from_dict(self, refit_weights, is_fp16):
        ...
        for trt_weight_name in refitter.get_all_weights():
            ...
            wt = refit_weights[trt_weight_name]

            # TRT expects host weights for set_named_weights in many setups;
            # keep behavior consistent and explicit.
            if is_fp16:
                wt = wt.half()

            wt = wt.contiguous().cpu()

            trt_datatype = trt.DataType.HALF if is_fp16 else trt.DataType.FLOAT
            ...
            # Apply refit (using the 2-arg overload, matching your current usage)
